...
public class ExtentReportListener implements ITestListener {
	...
	private static ExtentReports init() {
		...
		reporter.config().setReportName("Open Car: Automation Test Results");
	...
	public synchronized void onTestFailure(ITestResult result) {
		...
		test.get().fail(result.getThrowable(),
				MediaEntityBuilder.createScreenCaptureFromPath(DriverFactory.getScreenshot()).build());
		// test.get().fail(result.getThrowable(),
		// MediaEntityBuilder.createScreenCaptureFromPath(DriverFactory.getScreenshotinbase64()).build());
		test.get().getModel().setEndTime(getTime(result.getEndMillis()));
	}
